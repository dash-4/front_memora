        ...
        <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-1">
              Твой личный прогресс обучения на сегодня
            </p>
          </div>
          {stats?.streak > 0 && (
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          ...
          <div className="lg:col-span-7 grid grid-cols-2 gap-4">
            {statsCards.map((stat) => (
              ...
              >
                <div
                  className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-4 transition-transform group-hover:scale-110 ${colorMap[stat.color]}`}
                >
                  <stat.icon size={24} />
                </div>
            ...
            {decksWithDueCards.length > 0 ? (
              <div className="grid gap-3 sm:gap-4">
                {decksWithDueCards.map((deck) => (
                  <div
                    key={deck.id}
                    className="group bg-white flex flex-col sm:flex-row sm:items-center justify-between p-4 sm:p-5 rounded-[2rem] border border-slate-100 hover:border-blue-200 shadow-[0_8px_30px_rgb(0,0,0,0.02)] hover:shadow-[0_8px_30px_rgb(59,130,246,0.08)] transition-all active:scale-[0.98] sm:active:scale-100"
                  >
                    <div className="flex items-center gap-4 mb-4 sm:mb-0">
                      {/* Иконка с мягким фоном */}
                      <div
                        className="w-14 h-14 sm:w-12 sm:h-12 rounded-[1.25rem] flex items-center justify-center shrink-0 transition-transform group-hover:scale-110"
                        style={{
                          backgroundColor: `${deck.color || "#3B82F6"}15`,
                          boxShadow: `inset 0 0 0 1px ${deck.color || "#3B82F6"}20`,
                        }}
                      >
                        <BookOpen
                          size={24}
                      ...
                      <div className="min-w-0">
                        <h3 className="font-black text-slate-900 group-hover:text-blue-600 transition-colors truncate text-lg sm:text-base">
                          {deck.name}
                        </h3>
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="flex h-2 w-2 rounded-full bg-orange-400 animate-pulse" />
                          <p className="text-sm text-slate-500 font-bold uppercase tracking-tight opacity-80">
                    ...
                    <Button
                      ...
                      className="w-full sm:w-auto rounded-2xl bg-blue-600 hover:bg-slate-900 text-white font-black px-8 py-4 sm:py-2.5 shadow-lg shadow-slate-100 hover:shadow-blue-200 transition-all active:scale-95 shrink-0 text-sm sm:text-base"
                    >
                      Начать сессию
                    </Button>
                  </div>
              ...
              <div className="bg-white rounded-[2.5rem] border border-slate-100 p-10 text-center shadow-sm">
                <div className="w-20 h-20 bg-emerald-50 rounded-full flex items-center justify-center mx-auto mb-6">
                  <Target className="text-emerald-500" size={38} />
                </div>
                <h3 className="text-2xl font-black text-slate-900 tracking-tight">
                  Всё чисто!
                </h3>
                <p className="text-slate-500 max-w-[240px] mx-auto text-sm mt-3 font-medium leading-relaxed">
                  На сегодня долгов нет. Отличная работа над долгосрочной
                  памятью!
                </p>
              </div>
            )}
              ...
              {[
                ...
                <Link key={link.to} to={link.to} className="group">
                  <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm group-hover:shadow-md transition-all flex flex-col items-center gap-3">
                    <div
                      className={`p-3 rounded-xl ${link.bg} ${link.text} transition-transform group-hover:scale-110`}
                    >
                      <link.icon size={20} />
                    </div>
